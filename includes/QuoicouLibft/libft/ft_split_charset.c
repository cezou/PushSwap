...
/*   Updated: 2024/01/12 22:01:03 by cviegas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_sep(const char c, char *charset)
{
	int	i;
...
static int	lenbsep(const char *s, char *charset, int indice_s)
{
	int	i;
...
static int	ft_nb_mots(const char *str, char *charset)
{
	int	i;
	int	nb_mots;
...
char	**ft_split_charset(const char *str, char *charset)
{
	...
	while (str[j])
	{
		...
		{
			ret[k] = ft_calloc((lenbsep(str, charset, j) + 1), 1);
			i = 0;
